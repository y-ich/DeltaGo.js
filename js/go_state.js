...
class GoInput {
    constructor(position) {
        ...
        this.FEATURE_LIST = [
            ...
            ['turnsSince', 8],
            ['liberties', 8],
            ['captureSizeSelfAtariSizeLibertiesAfterMove', 8 + 8 + 8],
            ['ladderCapture', 1],
            ['ladderEscape', 1],
            ['sensibleness', 1],
            ['zeros', 1]
    ...
    ladderCapture(view) {
        for (let p = 0; p < this.position.BOARD_SIZE2; p++) {
            ...
            for (const o of move.string.opponents) {
...
class GoPosition {
    ...
    canEscape(string) {
        ...
        for (const o of string.opponents) { // アタリの相手の石を取る
            const s = this.stringAt(o);
            if (s.liberties.length == 1) {
                const escape = this.play(s.liberties[0]);
                if (!escape) {
                    continue;
                }
                const tryToCapture = this.play(string.liberties[0]);
                if (!tryToCapture) {
